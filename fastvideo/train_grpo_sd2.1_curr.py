...
def main(_):
    ...
    sampler = CurrDistributedSampler(
        ...
        seed = 123543,
        strategy="timestep", # "timestep", "balance", "cosine", "gaussian"
        total_steps=300,
        alpha=0.5,
        beta=0.8,
    )   
    ...
    for epoch, scenes in enumerate(loader):
        ...
        for i in range(0, len(expanded_prompts), batch_size):
            ...
            for j, image in enumerate(images):
                ...
                pil = pil.resize((768, 768))
                image_path = os.path.join("./images_same_sd2.1", f"image-{i}-{j}-rank-{dist.get_rank()}.jpg")
                pil.save(image_path)
        ...
        if dist.get_rank()==0:
            ...
            with open('./reward_sd2.1.txt', 'a') as f:  # 'a'模式表示追加到文件末尾
        ...
        if epoch != 0 and epoch % config.save_freq == 0: # 
            ...
            if accelerator.is_main_process:
                base_checkpoint_dir = "/mnt/tidal-alsh01/dataset/video_agent_data/ckpt/task/curr/20250713/ours_sd2.1"
